...
def test_feature_extraction(get_default_object):
    ...
    assert joined_temp.shape == (157, 8), "feature size has changed, has extract font, text, or box extraction changed"


    #get the right bounding boxes
    temp = pre_bi.to_box_coords(False)
    assert (int(temp["x2_jp"][2]),int(temp["y2_jp"][2]))==(350,1069), "remapping of coordinates does not match expected shape for unnormalized position"
...
def test_meta_feature_extraction(get_default_object):
    ...
    assert (int(temp["nn1"][0])==309), "nearest neighbor value does not match expected"
    assert (int(temp["nn2"][0]) == 667), "2nd nearest neighbor distance does not match expected"
    assert (int(temp["box_num"][0])==6), "number of boxes on page does not match expected"


    temp=pd.DataFrame((pre_bi.extract_squared_features(False)))
    assert(int(temp["x1_jp_squared"][0])==(410881)), "squared x1 coord feature does not match expected"
    assert(int(temp["height_jp_squared"][1]==(5184))), "squared height does not match expected"
...
def test_feature_prepare(get_default_object):
    ...
    assert (x.shape==(157,17)), "expected feature list for font_size  prediction is different from what was expected"
    assert (y.shape)==(157,1), "expected output for font_size is different from what was expected"

    assert x_feature_names == x.columns.tolist(), "expected feature list has changed for font feature creation"
    ...
    assert (x.shape==(157,17)), "expected feature list for bounding  prediction is different from what was expected"
    assert (y.shape)==(157,4), "expected output for bounding_prediction is different from what was expected"

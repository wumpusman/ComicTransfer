...
def test_bounding_prediction_train(get_text_processing_obj,get_basic_model):
    ...
    x_features_names = ["x1_jp","y1_jp","x2_jp","y2_jp"]  # features we wish to evaluate
    ...
    bound_model_predictor.fit(bound_model_predictor._x_names,bound_model_predictor._y_names)

    #expected output
    tr_scores, dev_scores = bound_model_predictor.score_cv()  # cross validation results

    np_test.assert_allclose(tr_scores,.65,.05), "baseline with almost no data, training results are different for bounding"
    np_test.assert_allclose(dev_scores, .63,.05), "baseline dev with almost no data , training results are different for bounding"

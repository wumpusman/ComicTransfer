...
def test_bounding_prediction_train(get_text_processing_obj, get_basic_model):
    ...
    x_features_names = ["x1_jp", "y1_jp", "x2_jp", "y2_jp",
                        "top_jp", "text_jp_len"]  # features we wish to evaluate
    ...
    np_test.assert_allclose(
        tr_scores, .7, .05), "training on small subset of data with linear model, training results are different for bounding"
    np_test.assert_allclose(
        dev_scores, .675, .07), "dev on small subset of data with linear model, training results are different for bounding"

    bound_model_predictor.fit(
        bound_model_predictor._x,
        bound_model_predictor._y,
        preprocess=True)

    results_good = bound_model_predictor.predict(
        bound_model_predictor._x, preprocess=True)
    results_bad = bound_model_predictor.predict(
        bound_model_predictor._x,
        preprocess=False)  # improperly normalized data

    assert np.abs(
        results_good[0][0] - 610) < 70, "bounding size for x1 has changed, check if preprocessing has changed"

...
driver_path = "C://users//egasy//downloads/chromedriver_win32//chromedriver" #was tested originally on windows
...
parser.add_argument("-d","--driver_path",help="path to chrome driver",default=driver_path)
parser.add_argument("-s","--save_dir",help="path to save files and images",default=save_dir)
parser.add_argument("-m","--manga_list",help="path to list of urls with each line referencing a link to one page per different manga",default=manga_files_path)
parser.add_argument("-n","--number_per_manga",help="max number of pages to attempt to save per page",default=int(max_pages),type=int)

#python scraping/main_extraction.py -m "../data/manga_list.txt" --number_per_manga 3

def main(driver_path:str,save_dir:str,manga_list_pth:str, max_pages_per_manga=1000):
    """
    ...
    for manga_initial_page in manga_list:
        ...
        while current < max_pages_per_manga: #max cap on links
            ...
            try: #not all pages are extractable, so just skip them
                results = extract_img_selenium.save_meta_data_eng_jp_pairs(driver, current_link)
                page_height = driver.find_elements_by_class_name("image-container")[0].size["height"]
                page_width = driver.find_elements_by_class_name("image-container")[0].size["width"]

                pd_results = pd.DataFrame(results)
                pd_results["manga"] = manga_name
                pd_results["page_width"] = page_width
                pd_results["page_height"] = page_height
                pd_results = extract_img_selenium.align_jp_and_en_boxes(pd_results)
                ...
                extract_img_selenium.save_eng_jp_pairs(driver,current_link,full_path,img_number)
            ...
            if len(next_link_ary)==0 or current>=max_pages_per_manga:
                ...
                tsv_to_save=os.path.join(save_dir,"{}{}".format(manga_name,".tsv"))
...
if __name__ == '__main__':
    ...
    main(args.driver_path,args.save_dir,args.manga_list,args.number_per_manga)
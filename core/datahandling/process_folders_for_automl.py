...
parser = argparse.ArgumentParser(description='setup for running and organizing files for automl')
parser.add_argument("-i","--imgdir",help="path to a jpg or png",default=default_path_img_dir)
parser.add_argument("-t","--tsvdir",help="path to font to be used",default=default_tsv_path)
parser.add_argument("-d","--destdir",help="path to destination folder",default=default_destination_dir)
parser.add_argument("-c","--destcsv",help="path to destination folder",default=default_destination_csv)
parser.add_argument("-p","--bucket",help="path where csv is to be placed",default=project_bucket)

def main(bucket_dir_path,img_directory_path:str,tsv_directory_path:str,destination_dir_path:str,destination_file_path:str):
    """
    ...
    file_names: list = [i for i in files if ("selenium.tsv" in i)] #the selenium files that have been extracted
    ...
    cuts = pd.qcut(mangas["font-size"], 13, labels=False) #binarize fonts, not used, but neccesary for the model
    ...
    all_pds = []
    type_of_analysis = ["TRAIN", "TRAIN", "VALIDATE", "TEST"] #50% train, 25% validate, 25% test
    for manga_name in manga_names:
        ...
        for page_id in page_numbers:
            ...
            if (os.path.isfile(path_to_file)):
                ...
                if (pd.DataFrame(process.extract_font_size()).shape[0] < 18): #automl has a cap of 20
                    ...
                    outside_params = final_data[
                        (final_data.x1_jp > 1) | (final_data.x2_jp > 1) | (final_data.y1_jp > 1) | (
                                    final_data.x2_jp > 1)]

                    final_data["full_id"] = 'gs://{}/{}'.format(bucket_dir_path,full_unique_img_id)
                    ...
                    destination_file=os.path.join(destination_dir_path,full_unique_img_id)
